...
class Admin extends \PluginCustomizer\Plugin_Customizer implements \PluginCustomizer\Plugin_Customizer_Interface {
	...
	function __construct() {
		...
		\PluginCustomizer\Plugin_Customizer::init(
			[
	...
	public function admin_menus() {
		add_action( 'admin_bar_menu', [ new \NetworkPortfolio\AdminMenu( $this ), 'add_admin_bar_customizer_url' ], 500 );
		add_action( 'admin_menu', [ new \NetworkPortfolio\AdminMenu( $this ), 'register_sub_menu' ] );
	}
	...
	public function plugin_customizer_add_templates() {
		...
		$default_url = [ 'url' => $default_template ];
		/**
	...
	public function plugin_customizer_previewer_postmessage_script() {
		...
		$deps      = [ 'boxshadow-css-hook', 'customize-preview', 'jquery' ];
		$version   = NETWORKPORTFOLIO_VERSION;
		$in_footer = 1;
		wp_enqueue_script( $handle, $src, $deps, $version, $in_footer );
	}
